...
describe('emailKeepDots(data)', () => {
	const emailStr = 'MAIL.SUBNAME@DOMAIN.TLD';

	itShouldFailStringAssertion(email);

	it('should call validator.normalizeEmail and keep dots', () => {
		email(emailStr);
		expect(validator.normalizeEmail).toHaveBeenCalledWith(emailStr);
	});

	it('should return a sanitized email', () => {
		expect(email(emailStr)).toEqual('mail.subname@domain.tld');
	});
});
